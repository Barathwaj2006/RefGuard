...
class ScamChainBuilder {
  build(extractedData, mismatch, threatAssessment, evidenceIds) {
    ...
    const makeNode = (type, entityRef, evRefs, state = 'OBSERVED', confidence = 0.95, provenance = 'CONTEXT_ENGINE_DERIVATION') => {
    ...
    if (extractedData.referralCodes && extractedData.referralCodes.length > 0) {
    ...
    if ((extractedData.vpas && extractedData.vpas.length > 0) || extractedData.upi) {
      const vpa = extractedData.vpas && extractedData.vpas.length > 0 ? extractedData.vpas[0] : (extractedData.upi ? extractedData.upi.pa : 'UPI_COLLECT');
      ...
      if (mismatch && (mismatch.status === 'DETECTED' || mismatch.payment_direction === 'OUTBOUND_DEBIT')) {
