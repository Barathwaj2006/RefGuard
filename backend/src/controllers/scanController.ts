...
export const scanContent = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // PII sanitization (we do this in analyzer too, but keeping controller clean)
    const sanitized = sanitizeText(scanReq.content_value);
    ...
    // The evidence pack contains RISK_SIGNAL with 'gemini_reasoning_applied'
    const isGeminiUsed = response.evidence_pack.items.some(
